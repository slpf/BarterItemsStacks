...
public class AdvancedStashSortingPatch : ModulePatch
{
    ...
    private static bool Prefix(List<Item> group, InventoryController inventoryController,
        List<IRaiseEvents> stagedOperations, bool runNetworkTransactions, ref Task<Error> __result)
    {
    ...
    private static async Task<Error> StackGroupFixed(List<Item> group, InventoryController inventoryController,
        List<IRaiseEvents> stagedOperations, bool runNetworkTransactions)
    {
        ...
        while (true)
        {
            ...
            var operation = InteractionsHandlerClass.TransferOrMerge(source, target, inventoryController, runNetworkTransactions);
            ...
            else
            {
                ...
                if (result.Failed) return new GClass1522(result.Error);
            }

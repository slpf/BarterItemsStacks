...
namespace BarterItemsStacksClient.Patches.Compatibility
{
    public class NestedQuickMoveStackOrderPatch : ModulePatch
    {
        ...
        private static IEnumerable<IContainer> GetContainers(CompoundItem compoundItem, Item item)
        {
            if (compoundItem is Stash stash)
            {
                return stash.Grids;
            }
        ...
        {
            IEnumerable<IContainer> containers = GetContainers(container, item);
